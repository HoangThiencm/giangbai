 ...
 * Hỗ trợ Multimodal (ảnh base64), chuyển đổi Model AI và lưu trữ cấu hình.
 */
...
class GeminiAPIManager {
  ...
  // Tải danh sách keys từ localStorage
  ...
  loadModelFromLocalStorage() {
    const savedModel = localStorage.getItem("khbd_gemini_model");
  ...
  async generateContent(prompt, images = [], systemRole = null, temperature = 0.3, signal = null) {
    if (!this.apiKeys || this.apiKeys.length === 0) {
      throw new Error("Chưa có API Key nào được cấu hình. Vui lòng bấm vào nút 'Quản lý API Key' trên thanh công cụ để nhập ít nhất 1 key.");
    }

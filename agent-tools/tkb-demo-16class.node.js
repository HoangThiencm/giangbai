/**
 * Demo xáº¿p TKB: 16 lá»›p, ~30 GV (16 * 1.9)
 * Cháº¡y: cscript //nologo agent-tools/tkb-demo-16class.js
 * Port thuáº­t toÃ¡n tá»« thoikhoabieu.html (solveByConstraints + precheck)
 */
...
// ===================== DEMO DATA: 16 lá»›p Â· 30 GV (16 * 1.9) =====================
function buildDemoData() {
  ...
  // 16 classes: 4 khá»‘i Ã— 4 lá»›p â€” half morning, half both for capacity
  var classNames = [
    '6A', '6B', '6C', '6D',
    '7A', '7B', '7C', '7D',
    '8A', '8B', '8C', '8D',
  ...
  for (var i = 0; i < classNames.length; i++) {
    // 1 buá»•i sÃ¡ng: capacity 30; 2 buá»•i: 54 â€” dÃ¹ng morning cho táº¥t cáº£ Ä‘á»ƒ mÃ´ phá»ng THCS phá»• biáº¿n
    // nhÆ°ng gÃ¡n ~28 tiáº¿t/lá»›p nÃªn cáº§n morning only OK
    addClass({ name: classNames[i], shift: 'morning', homeRoom: classNames[i] });
  }

  // 30 teachers = round(16 * 1.9)
  // PhÃ¢n theo mÃ´n thá»±c táº¿ THCS, má»—i GV dáº¡y nhiá»u lá»›p
  var teacherDefs = [
    // ToÃ¡n: 4 GV Ã— 4 lá»›p Ã— 4 tiáº¿t = 16 tiáº¿t/GV
    ...
    // Tin: 2 (16 lá»›p Ã— 2 tiáº¿t = 32 â†’ 16 tiáº¿t/GV)
  ...
  if (teacherDefs.length !== 30) {
    console.log('ERROR: expected 30 teachers, got ' + teacherDefs.length);
  }
  ...
  // PhÃ²ng BM: má»—i lab â‰¥ 2 phÃ²ng Ä‘á»ƒ 16 lá»›p Ã— 2 tiáº¿t (32) â‰¤ 2Ã—30 Ã´ sÃ¡ng
  ...
  for (var ci = 0; ci < state.classes.length; ci++) {
    ...
    for (var si = 0; si < curriculum.length; si++) {
      ...
      if (!teacher) {
        ...
        console.log('Missing teacher for ' + cur.subject);
        continue;
      }
      addAssignment({
        classId: cl.id,
...
function main() {
  ...
  console.log('=== DEMO TKB 16 lop x 30 GV (16*1.9) ===');
  console.log('Lop: ' + meta.classes + ' | GV: ' + meta.teachers + ' | Ty le GV/lop: ' + meta.ratio);
  console.log('Phong BM: ' + meta.rooms + ' | Dong phan cong: ' + meta.assignments);
  console.log('Cau hinh: ' + state.rules.days.join(',') + ' | Sang ' + state.rules.morningPeriods + ' tiet | Chieu ' + state.rules.afternoonPeriods + ' | max mon/ngay ' + state.rules.maxSameSubjectDay);

  var lessons = buildLessonUnits();
  var domains = buildBaseDomains(lessons);
  console.log('Tong tiet can xep: ' + lessons.length);
  console.log('Capacity 1 lop morning: ' + slotsForClass(state.classes[0]).length + ' o');

  var loads = teacherLoadSummary(lessons);
  console.log('--- Tai GV (top 10) ---');
  for (var i = 0; i < Math.min(10, loads.length); i++) {
    console.log('  ' + loads[i].name + ' (' + loads[i].subject + '): ' + loads[i].load + '/' + loads[i].max);
  }

  var pre = precheckSchedule(lessons, domains);
  if (pre.length) {
    console.log('PRECHECK FAIL: ' + pre.length + ' van de');
    for (var p = 0; p < pre.length; p++) console.log('  - ' + pre[p].message);
    return;
  }
  console.log('Precheck: OK');

  console.log('Dang chay solver...');
  ...
  console.log('Da xep: ' + placed + '/' + lessons.length + ' (' + pct + '%)');
  console.log('Nodes: ' + solved.nodes + ' | deepest: ' + solved.deepest);
  console.log('Thoi gian: ' + ((t1 - t0) / 1000) + 's');
  if (solved.beauty) {
    console.log('Beauty score: ' + solved.beauty.score +
  ...
  if (solved.blockedLesson) {
    console.log('Diem nghen: ' + solved.blockedLesson.subject + ' - ' +
  ...
  console.log('Hard-constraint violations tren o da xep: ' + violations);
}

...
(function () {
    ...
    // Kiểm tra manifest deploy độc lập với cơ chế khóa DevTools để mọi trang đều nhận bản mới.
    var appVersionStorageKey = '__system_app_version__';
    var appVersionReloadAtKey = '__system_app_version_reload_at__';
    var appVersionReloadDebounceMs = 10000;
    var appVersionCheckInFlight = false;

    function getVersionValue(payload) {
        var version = payload && payload.version;
        return typeof version === 'string' && version.trim() ? version.trim() : '';
    }

    function checkAppVersionUpdate() {
        if (appVersionCheckInFlight || typeof window.fetch !== 'function') {
            return Promise.resolve(false);
        }

        appVersionCheckInFlight = true;
        var manifestUrl = '/version.json?_t=' + Date.now();
        return window.fetch(manifestUrl, { cache: 'no-store' })
            .then(function (response) {
                if (!response || !response.ok) throw new Error('Version manifest unavailable');
                return response.json();
            })
            .then(function (manifest) {
                var serverVersion = getVersionValue(manifest);
                if (!serverVersion) return false;

                var currentVersion = storageGet(localStorage, appVersionStorageKey);
                if (!currentVersion) {
                    storageSet(localStorage, appVersionStorageKey, serverVersion);
                    return false;
                }
                ...
                storageSet(localStorage, appVersionStorageKey, serverVersion);
                var now = Date.now();
                var lastReloadAt = Number(storageGet(sessionStorage, appVersionReloadAtKey) || 0);
                if (now - lastReloadAt < appVersionReloadDebounceMs) return false;

                storageSet(sessionStorage, appVersionReloadAtKey, String(now));
                try {
            ...
            .catch(function () {
                // Mất mạng hoặc manifest tạm thời chưa sẵn sàng không được ảnh hưởng tới trang.
                return false;
            })
    ...
    function initAutoUpdateChecker() {
        ...
        document.addEventListener('visibilitychange', function () {
            if (!document.hidden) checkAppVersionUpdate();
        });
        window.addEventListener('focus', checkAppVersionUpdate, { passive: true });
        setInterval(checkAppVersionUpdate, 60000);
    }

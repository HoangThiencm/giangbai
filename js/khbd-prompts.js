...
(function (global) {
  if (typeof global.getPromptTemplate === "function" && global.PROMPTS && global.PROMPTS.GENERATE_OBJECTIVES) {
    return;
  }
  ...
  (function () {
    ...
    - CỘT TRÁI — KỊCH BẢN THỰC CHIẾN PHÂN VAI RÕ RÀNG (ngăn các bước và vai trò bằng <br>):
      ...
      + Định mức PPDH/KTDH theo số tiết (TIME-BUDGET GATE): CHỈ áp dụng cho các PPDH/KTDH ĐÃ ĐƯỢC CHỌN trong bối cảnh sư phạm. Nếu người dùng không chọn hoặc đã bỏ tick toàn bộ PPDH/KTDH, chỉ tổ chức trực tiếp, vấn đáp gợi mở và luyện tập cơ bản; TUYỆT ĐỐI KHÔNG ghi hoặc tự đặt tên bất kỳ PPDH/KTDH nào.
        * Bài 1 tiết (<= 45 phút): nếu có chọn, tối đa 1 PPDH chủ đạo và 1–2 KTDH nhẹ; không tự thêm kỹ thuật không được chọn. CẤM kỹ thuật nặng khi không được chọn.
        * Bài 2 tiết (90 phút): nếu có chọn, tối đa 2 PPDH và 2–3 KTDH đã chọn.
        * Bài từ 3 tiết: nếu có chọn, tối đa 2 PPDH và 3–4 KTDH đã chọn.
      + Trong từng bước, CHỈ nêu tên Kỹ thuật/Phương pháp dạy học khi chúng đã được chọn trong bối cảnh sư phạm. Nếu không có lựa chọn, PHÂN VAI RÕ RÀNG nhưng TUYỆT ĐỐI KHÔNG ghi tên hoặc tự đặt tên KTDH/PPDH:
      ...
      + KỊCH BẢN TÍCH HỢP NĂNG LỰC SỐ (NLS) VÀ NĂNG LỰC AI THỰC CHIẾN GẮN MÔN HỌC & TÍCH HỢP THEO MÔN (CHỈ khi được GV bật trong bối cảnh sư phạm):
        ...
        * BẮT BUỘC: Các vị trí tích hợp NLS và AI phải được in đậm và in nghiêng (dùng cú pháp Markdown ***...***).
    ...
    5. Khung Năng lực Trí tuệ Nhân tạo (AI) theo Quyết định số 2422/QĐ-BGDĐT (4 Miền A, B, C, D theo từng lớp cụ thể).
    6. Khung Năng lực Số theo Thông tư số 02/2025/TT-BGDĐT và Công văn số 3456/BGDĐT-GDPT (gồm 5 Miền: Khai thác dữ liệu & thông tin, Giao tiếp & hợp tác trong môi trường số, Sáng tạo nội dung số, An toàn, Giải quyết vấn đề; Lớp 6–7 dùng dải Trung cấp 1 (TC1), Lớp 8–9 dùng dải Trung cấp 2 (TC2)).
    ...
    - ĐẾM SỐ TIỂU MỤC KIẾN THỨC LỚN TRONG SGK: Chỉ ánh xạ đúng các Mục lớn (Đơn vị kiến thức cốt lõi chính thức trong mục lục SGK, thông thường 1–3 mục lớn, tối đa 4 mục). Bạn BẮT BUỘC PHẢI chia Hoạt động B thành đúng N hoạt động con tương ứng 1-1:
      ### Hoạt động 2.1: [Tên nguyên văn đề mục trong SGK] (... phút)
      ### Hoạt động 2.2: [Tên nguyên văn đề mục trong SGK] (... phút)
      ...
    ...
    ### Hoạt động 2.1: [Tên nguyên văn đề mục trong SGK] (15 phút)
    #### a) Mục tiêu:
    ...
    **HỌ VÀ TÊN THÀNH VIÊN: .....................................................................................**  
    ...
    function extractTextbookSubsections(content) {
      ...
      function stripHeadingMarker(title) {
        return String(title || "")
      ...
      function isIgnoredCore(title, allowGenericWord) {
        ...
        if (/^(?:Tổng quan|Khung kiến thức|Chuỗi hoạt động|Hệ thống bài tập|Đề xuất|Yêu cầu cần đạt|Tiến trình|Thiết bị|Hoạt động mở đầu|Hoạt động hình thành|Hoạt động luyện tập|Hoạt động vận dụng|Đánh giá|Hồ sơ|Hướng dẫn về nhà|Ngữ cảnh SGK|Đề mục SGK|Cấu trúc kiến thức)\b/i.test(title)) return true;
    
        // 5. Loại trừ các từ đơn lẻ generic (không áp dụng khi title đã có chỉ số SGK)
        if (!allowGenericWord && /^(?:định nghĩa|khái niệm|quy tắc|công thức|chú ý|ví dụ|bài tập|nhiệm vụ|bước\s*\d+|phương pháp|kỹ thuật)$/i.test(title)) return true;
    
        return false;
      }
    
      function isIgnored(title) {
        if (isIgnoredCore(title, false)) return true;
        const stripped = stripHeadingMarker(title);
        return stripped !== title && isIgnoredCore(stripped, true);
      }
      ...
      // Chiến lược 0: Dòng "Đề mục:" do formatCanvasTextbookContext xuất — giữ nguyên văn 100%.
      const deMucRegex = /(?:^|\n)\s*(?:[-*+•]\s+)?(?:#{1,6}\s+)?(?:\*\*)?Đề mục:\s*(.+)/gi;
      let match;
      while ((match = deMucRegex.exec(text)) !== null) {
        addSub(results.length + 1, match[1]);
      }
      ...
      while ((match = romanRegex.exec(text)) !== null) {
        ...
        if (/^(?:I|II|III|IV|V|VI|VII|VIII|IX|X)$/.test(romanStr)) {
          addSub(results.length + 1, `${romanStr}. ${title}`);
        }
      }
      ...
      if (templateKey === 'GENERATE_ACTIVITY_B' || templateKey === 'GENERATE_ACTIVITIES_AD' || templateKey === 'GENERATE_ACTIVITIES_AE') {
        if (subsections && subsections.length > 0) {
          const subListStr = subsections.map((s, idx) => {
            ...
            return `+ Tiểu mục ${s.index}: "${s.title}" -> BẮT BUỘC sinh: ### Hoạt động 2.${s.index}: ${s.title} (${subTime})`;
          }).join('\n');
    ...
    global.isEnglishSubject = isEnglishSubject;
    global.isReviewOrPracticeLesson = isReviewOrPracticeLesson;
    global.isPracticeOrReviewLesson = isPracticeOrReviewLesson;
    global.getSystemRole = getSystemRole;
    global.getPromptTemplate = getPromptTemplate;
    global.calculateActivityTimeBudgets = calculateActivityTimeBudgets;
    global.extractTextbookSubsections = extractTextbookSubsections;
    global.normalizeTextbookSubsectionProfiles = normalizeTextbookSubsectionProfiles;
    global.extractTextbookLessonMap = extractTextbookLessonMap;
    global.getGeneralCompetenciesForSubject = getGeneralCompetenciesForSubject;
    global.formatGeneralCompetenciesGuide = formatGeneralCompetenciesGuide;

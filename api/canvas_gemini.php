...
if($tier==='high_reasoning'&&$userKeys)foreach($userKeys as $index=>$key){$attempt=canvas_gemini_call($preferredModel,$key,$encoded,$userAttemptTimeout);if($attempt['ok'])respond(['ok'=>true,'body'=>$attempt['body'],'model'=>$preferredModel,'tier'=>'user_key','key_index'=>$index+1,'total_user_keys'=>count($userKeys),'rotation_count'=>count($quotaKeyIndexes),'quota_key_indexes'=>$quotaKeyIndexes,'fallback_used'=>false]);$last=$attempt;if(canvas_is_quota_failure($attempt)){$quotaKeyIndexes[]=$index+1;$fallbackReason='user_key_quota';continue;}$fallbackReason=canvas_is_timeout_or_transient_failure($attempt)?'user_key_timeout':'user_key_error';break;}
foreach($systemKeys as $key){$remaining=(int)floor($deadline-microtime(true)-3);if($remaining<10)break;$systemAttemptTimeout=min(55,$remaining);$attempt=canvas_gemini_call('gemini-3-flash-preview',$key,$encoded,$systemAttemptTimeout);if($attempt['ok'])respond(['ok'=>true,'body'=>$attempt['body'],'model'=>'gemini-3-flash-preview','tier'=>$tier==='heavy_io'?'system_key':'fallback_system','fallback_used'=>$tier==='high_reasoning','fallback_reason'=>$tier==='high_reasoning'?$fallbackReason:null,'total_user_keys'=>count($userKeys),'rotation_count'=>count($quotaKeyIndexes),'quota_key_indexes'=>$quotaKeyIndexes]);$last=$attempt;}

...
try {
    ...
    if ($method === 'GET') {
        ...
        if ($year !== '') {
            $q = $pdo->prepare('SELECT * FROM teacher_ppct_catalogs WHERE owner_user_id=? AND subject=? AND grade=? AND academic_year=? LIMIT 1');
            $q->execute([(int)$teacher['id'], $subject, $grade, $year]);
        } else {
            $q = $pdo->prepare('SELECT * FROM teacher_ppct_catalogs WHERE owner_user_id=? AND subject=? AND grade=? ORDER BY updated_at DESC,id DESC LIMIT 1');
            $q->execute([(int)$teacher['id'], $subject, $grade]);
        }
        $row = $q->fetch();
        if (!$row) canvas_ppct_reply(['ok' => true, 'catalog' => null]);
        $row['rows'] = json_decode($row['rows_json'], true);
        $row['source'] = json_decode($row['source_json'], true);
        unset($row['rows_json'], $row['source_json']);
        canvas_ppct_reply(['ok' => true, 'catalog' => $row]);
    }
    ...
    $q = $pdo->prepare('INSERT INTO teacher_ppct_catalogs (owner_user_id,subject,grade,academic_year,rows_json,source_json) VALUES (?,?,?,?,?,?) ON DUPLICATE KEY UPDATE rows_json=VALUES(rows_json),source_json=VALUES(source_json),updated_at=CURRENT_TIMESTAMP');
    $q->execute([(int)$teacher['id'], $subject, $grade, $year, $rj, $sj]);
    $q = $pdo->prepare('SELECT id,subject,grade,academic_year,updated_at FROM teacher_ppct_catalogs WHERE owner_user_id=? AND subject=? AND grade=? AND academic_year=?');
    $q->execute([(int)$teacher['id'], $subject, $grade, $year]);

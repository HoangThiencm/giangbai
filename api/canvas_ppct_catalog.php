...
try {
    ...
    if ($method === 'GET') {
        ...
        if (!$schoolProvided) { $q=$pdo->prepare('SELECT id,subject,grade,academic_year,school_name,updated_at FROM teacher_ppct_catalogs WHERE owner_user_id=? AND subject=? AND grade=? ORDER BY updated_at DESC,id DESC');$q->execute([(int)$teacher['id'],$subject,$grade]);canvas_ppct_reply(['ok'=>true,'profiles'=>$q->fetchAll()]); }
        $base=' FROM teacher_ppct_catalogs WHERE owner_user_id=? AND subject=? AND grade=? AND school_name=?';if($year!==''){$q=$pdo->prepare('SELECT *'.$base.' AND academic_year=? ORDER BY updated_at DESC,id DESC LIMIT 1');$q->execute([(int)$teacher['id'],$subject,$grade,$school,$year]);$row=$q->fetch();if(!$row){$q=$pdo->prepare('SELECT *'.$base." AND academic_year='' ORDER BY updated_at DESC,id DESC LIMIT 1");$q->execute([(int)$teacher['id'],$subject,$grade,$school]);$row=$q->fetch();}}else{$q=$pdo->prepare('SELECT *'.$base." AND academic_year='' ORDER BY updated_at DESC,id DESC LIMIT 1");$q->execute([(int)$teacher['id'],$subject,$grade,$school]);$row=$q->fetch();}canvas_ppct_reply(['ok'=>true,'catalog'=>$row?canvas_ppct_catalog_row($row):null]);
    }

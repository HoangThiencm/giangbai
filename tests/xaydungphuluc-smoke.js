...
assert.equal(ppctEditing.manualTietCT,'20\n21','updatePpctField must still allow a direct Tiết CT override');
...
assert.equal(ppctEditing.table[0][2],'1\n2','reordering must recalculate cumulative Tiết CT from the first lesson');
assert.equal(ppctEditing.table[1][2],'3\n4\n5','reordering must recalculate every later Tiết CT cumulatively');
assert.equal(ppctEditing.table[0][3],'1','reordering must recalculate the first lesson week');
assert.equal(ppctEditing.table[1][3],'1\n2','reordering must allocate a cross-week lesson to both weeks');
assert.equal(ppctEditing.rows[1].week,'1\n2','recalculation must synchronize canonical PPCT rows');
...
assert.equal(ppctInsertDelete.afterInsert.table[1][2],'3','inserting must immediately recalculate cumulative Tiết CT');
assert.equal(ppctInsertDelete.afterInsert.table[2][2],'4','inserting must shift later Tiết CT values');
...
assert.equal(ppctInsertDelete.afterHeader.tiet,'4','a header must not consume a curriculum period');

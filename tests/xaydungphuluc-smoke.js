...
['saveDraftToServer','loadDraftFromServer','buildDraftPayload','applyDraftPayload','api/user_phuluc_draft.php','💾 Lưu lên CSDL','📂 Tải từ CSDL','id="draftStatus"','aiSelectedLessonIds','sgkCompactContext','sgkKnowledgeBase'].forEach(has);
['user_phuluc_drafts','LONGTEXT','$_SESSION[\'user_id\']','UNIQUE KEY uniq_user_phuluc_draft_user','ON DUPLICATE KEY UPDATE','draft_data','updated_at'].forEach(value=>assert(draftApi.includes(value),`draft API missing ${value}`));

...
['id="canvasAiStatus"','id="canvasAiStatusMessage"','role="status"','aria-live="polite"','function showCanvasAiStatus','function hideCanvasAiStatus','canvas-ai-status','pointer-events:none','[AI] Đang sinh Phụ lục','showCanvasAiStatus(statusMessage,{state:\'complete\''].forEach(text=>assert(target.includes(text),`missing floating AI status requirement: ${text}`));
...
assert(sliceFunction('readGeminiResponse').includes("envelope?.fallback_reason==='user_key_timeout'"),'Canvas must distinguish a key timeout from quota fallback in its log');

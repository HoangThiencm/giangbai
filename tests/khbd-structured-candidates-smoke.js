...
assert.match(app, /foldedAnchor\.length < 12 \|\| !foldedSource\.includes\(foldedAnchor\)/, "Neo lệch dấu câu chỉ nhận khi fold ≥ 12 và nằm trong OCR");
assert.match(app, /anchorTerms\.some\(word => taskText\.includes\(word\)\)/, "Nhiệm vụ phải gắn neo SGK");
assert.match(app, /resolveCandidateEntry\(id, byId, byCode\)/, "Phải khớp id catalog hoặc mã chính thức duy nhất");

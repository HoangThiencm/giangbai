...
6. **Quản trị tài khoản (`admin.html`)**
   ...
   - **Cấu hình AI**: Cloudflare Worker URL và `APP_SHARED_SECRET` nằm trong `api/config.php` server-side. Admin chọn model Workers AI (`@cf/qwen/qwen3-30b-a3b-fp8` hoặc `@cf/meta/llama-3.2-3b-instruct`); PHP gửi model đã chọn cho Worker, Worker chỉ cho phép danh sách model an toàn. Gemini và DeepSeek/ShopAIKey có công tắc độc lập; khi tắt provider, key vẫn được giữ trong `global_config.json` nhưng backend bỏ qua provider đó — dùng để kiểm thử Worker độc lập.

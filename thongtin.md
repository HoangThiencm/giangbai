...
- **Chia sẻ link bài học:** URL trên thanh địa chỉ tự cập nhật `?lessonId=<id>` mỗi khi chuyển bài (`history.replaceState`). Giáo viên có nút **📋 Sao chép link** trên toolbar soạn bài → copy URL dạng `lotrinhtoanX.html?lessonId=N` vào clipboard, gửi qua Zalo/nhóm lớp. HS bấm link → đăng nhập (nếu chưa) → vào thẳng bài đó. Bài phải `is_published` để HS thấy nội dung.

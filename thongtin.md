...
5. **Backend API (`api/`)**
   ...
   - `ai_explain.php`: Ưu tiên **AI học tập nhẹ** chạy ngay trên PHP/hosting: dò khái niệm, công thức và ví dụ từ nội dung bài giáo viên đã soạn, không dùng API key và không gửi dữ liệu ra ngoài. Nếu không tìm được nội dung đủ phù hợp thì gọi Gemini; Gemini hết quota/lỗi thì fallback ShopAIKey (`https://api.shopaikey.com/v1/chat/completions`, mặc định model `deepseek-v4-flash`). Cấu hình bật/tắt AI nhẹ và key/model dự phòng trong `admin.html` → `global_config.json` trên hosting.
...
6. **Quản trị tài khoản (`admin.html`)**
   ...
   - **Cấu hình AI**: AI học tập nhẹ được bật mặc định, có thể tắt trong admin; Gemini và DeepSeek/ShopAIKey cũng có công tắc độc lập. Khi tắt provider, key vẫn được giữ trong `global_config.json` nhưng backend bỏ qua provider đó — dùng để kiểm thử AI nhẹ độc lập. Banner `#adminConfigStorageBanner` hiển thị nguồn (hosting/GitHub/file), trạng thái AI nhẹ, số key Gemini, lần lưu gần nhất. `localStorage.global_gemini_keys` chỉ là bản copy khi HS mở `index.html`.
...
- **AI Explain:** Luồng trả lời là **bộ giải Toán nhẹ → tra nội dung bài học → Gemini → ShopAIKey**. Hiện bộ giải Toán xử lý phương trình bậc nhất một ẩn dạng tổng/hiệu/nhân đơn giản, độc lập với bài đang mở (vd. `x + 1 = 5`); bước tra bài học chỉ dùng khi có ít nhất hai từ khóa khớp để tránh lấy nhầm đoạn. AI nhẹ không phải mô hình ngôn ngữ lớn: câu hỏi mở hoặc không xử lý được sẽ tự chuyển provider tiếp theo. Đầu ra từ AI cần được loại bỏ markdown thô trước khi hiển thị. Frontend gọi `typesetMath()` sau khi chèn text vào bong bóng chat để đảm bảo công thức toán học được render đúng chuẩn. Bong bóng được gắn theo ngữ cảnh nút vừa bấm và người dùng có thể đóng bằng nút `x`.

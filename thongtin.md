...
- **AI Explain:** Luồng trả lời là **AI học tập nhẹ → Gemini → ShopAIKey**. AI nhẹ không phải mô hình ngôn ngữ lớn: nó chỉ trả lời khi truy xuất được phần liên quan từ chính bài học, vì vậy phù hợp với hosting thường; câu hỏi mở hoặc không tìm được dữ liệu sẽ tự chuyển provider tiếp theo. Đầu ra từ AI cần được loại bỏ markdown thô trước khi hiển thị. Frontend gọi `typesetMath()` sau khi chèn text vào bong bóng chat để đảm bảo công thức toán học được render đúng chuẩn. Bong bóng được gắn theo ngữ cảnh nút vừa bấm và người dùng có thể đóng bằng nút `x`.

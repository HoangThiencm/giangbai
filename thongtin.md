...
6. **Quản trị tài khoản (`admin.html`)**
   ...
   - **Cấu hình AI**: Cloudflare Worker URL và `APP_SHARED_SECRET` nằm trong `api/config.php` server-side. Gemini và DeepSeek/ShopAIKey có công tắc độc lập trong Admin; khi tắt provider, key vẫn được giữ trong `global_config.json` nhưng backend bỏ qua provider đó — dùng để kiểm thử Worker độc lập. Banner `#adminConfigStorageBanner` hiển thị nguồn (hosting/GitHub/file), số key Gemini, lần lưu gần nhất. `localStorage.global_gemini_keys` chỉ là bản copy khi HS mở `index.html`.

  ...
  function solveByConstraints(lessons, domains) {
      ...
      function solveBySlotScan(seed = 0) {
          ...
          function scoreOption(index, opt) {
              ...
              if (!cPeriods.length) {
                  score += opt.period === start ? -35 : 40 + (opt.period - start) * 18;
              } else {
                  const minP = Math.min(...cPeriods);
                  const maxP = Math.max(...cPeriods);
                  if (opt.period === maxP + 1 || opt.period === minP - 1) score -= 28;
                  else if (opt.period > maxP + 1 || opt.period < minP - 1) score += 48;
                  if (minP > start) score += 30;
              }
              // jitter seed
          ...
          // Phase 1: fill theo ô — mọi lớp đồng thời, gói từ tiết đầu
          // seed chẵn: T2→T7; seed lẻ: đảo ngày; seed % 3 === 2: tiết ngược (đa dạng nghẽn GV)
          ...
          // Phase 2: residual — còn lại gán ô trống tốt nhất
      ...
      if (isLarge || isHuge) {
          ...
          for (let s = 0; s < seedCount; s++) {
              ...
              if (wave.ok) {
                  let assign = wave.assignments;
                  // Soft: dồn orphan + giữ gói lớp (không phá hard)
                  const repaired = reduceTeacherOrphans(assign, isHuge ? 5000 : 9000);
                  assign = repaired.assignments;
                  ...
                  return {
                      ...
                      deepest: lessons.length,
                      blockedLesson: null,
                      beauty: { ...evaluateBeauty(lessons, assign), beautifyMoves: repaired.moves },
                  };
              }
              // đủ 100% sớm thì dừng; gần đủ thì tiếp tục seed (chain repair có thể khác)
              if (best && best.placed === lessons.length) break;
          }
          totalNodes += totalWaveNodes;
          if (best) {
              deepest = Math.max(deepest, best.placed || 0);
              rememberPartial(best.assignments, evaluateBeauty(lessons, best.assignments));
              // Large: không rơi CSP nặng — trả partial tốt nhất (tránh treo 1–2 phút)
              state._packSoftMode = false;
      ...
      if (bestSolution) {
          ...
          // Medium/small: dồn tiết mồ côi GV sau beautify (large đã xử lý ở wave)
          if (!isLarge && !isHuge) {
